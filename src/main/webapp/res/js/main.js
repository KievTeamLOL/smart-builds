$('.gamecmp').click(function () {
    if (!($(".item_block").hasClass("active_item_block"))) {
        alert('You didn`t activate any block. Click on the block to make it active.');
        return 0;
    }
...
$(".sort_block>ul>li").click(function () {
    ...
    $(".gamecmp").each(function () {
        if ($(this).attr("tag") == $(".active_sort_block").attr("tag")) {
            $(this).css({display: "block"})
        }
    });

    $(".gamecmptext").css({display: "none"});

    $(".gamecmptext").each(function () {
        if ($(this).attr("tag") == $(".active_sort_block").attr("tag")) {
            $(this).css({display: "block"})
        }
    });

    $(".gamecmpblock").css({display: "none"});

    $(".gamecmpblock").each(function () {
        if ($(this).attr("tag") == $(".active_sort_block").attr("tag")) {
            $(this).css({display: "block"})
        }
    });
